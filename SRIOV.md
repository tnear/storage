...
See also: [NVMe](Nvme.md) and [PCIe](PCIe.md)

> ### Note: Single root
> *Single root* means that the physical device and all its PFs and VFs are underneath one hosts's PCIe *root complex*.
> ```
> One physical host
...
A *physical function* is a full-featured PCIe function: the "real" one. It can do everything the device supports: configuration, management, and data transfer. It's also the function that controls SR-IOV itself. It tells the hardware to create virtual functions. The hypervisor owns the physical function.
...
Suppose there is an 8 TB SSD. Creating four VFs does *not* automatically produce four 2 TB disks. SR-IOV only creates PCIe interfaces: it does not manage storage capacity.

Storage capacity is through NVMe namespaces. The administrator configures which namespaces are accessible through which functions.
